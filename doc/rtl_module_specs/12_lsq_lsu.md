...
- checkpoint 保存独立于 LQ/SQ allocation fire；普通整数分支即使本 bundle 不分配
  访存槽，也会保存当前两条 log tail。非零 keep-count 只允许与真实 allocation fire 同拍。
...
- `test/tb_lsq_allocator.sv` 覆盖双 LQ/SQ reservation、payload 保持、fire/cancel、
  checkpoint keep-count、零访存分配 checkpoint、多周期 rollback、release、exception flush
  和无效 checkpoint no-op。
- QuestaSim：`tb_lsq_allocator` 及当前 28 项回归通过，`Errors: 0, Warnings: 0`。
- 零分配 checkpoint 修正后用户 OOC：200 MHz / 5.000 ns 下 WNS = +2.138 ns，冻结。
