...
### 5.3 2026-07-09 mem issue allowed / candidate 出口切分

针对 `91f9c06` 在 175 MHz full route 上暴露的主路径：

```text
mem_issue_queue.rob_id/candidate
  → older-store wait / mem_issue_allowed
  → issue_arbiter high-fanout select
  → issue_uop*_o 宽字段
```

当前分支 `timing/mem-issue-allowed-and-candidate-cut` 再切两刀，不改 ISA/访存序契约：

1. **`mem_issue_allowed` 寄存**（`backend_lsu_cluster` / `backend_mdu_cluster`）
   - 组合结果为 `mem_issue_allowed_d`，打拍后为 `mem_issue_allowed_q` 再送给 arbiter。
   - `mem_candidate_reselect_q` 仍跟**新鲜** `~allowed_d` 同拍寄存，避免 held blocked load 永久卡住。
   - ROB 年龄比较继续以当前 ROB head 为参考。
2. **Issue Arbiter P2 不再消费 live combo allowed**
   - LSU `endpoint_ready` 使用 C0 已寄存的 `mem_issue_allowed_q`。
   - 语义偏保守：stale allow=0 只产生 bubble；held load 在 older store 地址齐后 allow 单调 0→1。
   - 宽 `issue_uop` 打包仍只由 P1 `selected_valid_q` 驱动（`ef3d2f6` 解耦保持）。
3. **IQ candidate 宽 payload 寄存**（`issue_queue`）
   - S1 fill/reselect 时装载 `candidate_uop_q`。
   - hold 期间只刷新 `src1_ready/src2_ready/branch_mask` 动态位，静态字段保持。
   - 出口不再做 `payload_q[candidate_slot]` 组合 MUX。

验证（本分支）：

- `tb_issue_queue`、`tb_issue_arbiter`（TB 流水拍数已对齐 C0/P0/P1/P2 寄存边界）
- `tb_backend_int/lsu/mdu_cluster`
- 官方支持集 `51/51 PASS`（排除 `fence_i`）
- JYD2025 COE smoke PASS，`LED=0x0002_0001`
- 自定义 SoC 指令 `39/39 PASS`

完整核 5.000 ns post-route 时序仍需在实现环境复测；本轮 RTL 目标是打断上述 combo 锥。
