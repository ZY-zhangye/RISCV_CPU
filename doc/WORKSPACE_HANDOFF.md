...
### 12. IF/ID 与 Core 顶层串联

- `core_top.sv` 已连接 IF→ID→`backend_top`，外部保留同步 64-bit IMEM、typed DMEM、中断和乘除法 IP 接口；
- IF/ID 已改用 typed `recover_event_t`，IF 直接接收 typed `branch_update_t`；
- `RECOVER_FENCE_I` 在提交后清除年轻流水并从 FENCE.I 的 `PC+4` 重取；
- BRU 实际下一 PC 随 ROB completion/commit 传递，Backend 内部 `retire_next_pc` 取代外部 `interrupt_pc_i`；
- `unified_memory_model.sv` 提供统一 IMEM/DMEM word array；DMEM 请求先经过 Core 外寄存级，Load 不允许组合返回；
- `tb_core_single_instruction.sv` 已通过 ADDI、ADD、LW、SW 及外部 Load 时序断言；官方 HEX 回归尚未开始。
- `tb_core_branch_instructions.sv` 已通过六类条件分支、正确预测 not-taken、JAL/JALR、链接值、恢复目标及错误路径 RRAT 隔离；
...
`backend_top.sv` 输入 typed `ds_rn_bundle_t`、DMEM response、三类中断与 vendor-neutral 乘除法 IP 返回；输出 `rn_allowin`、DMEM/IP 请求、`recover_event_t`、`branch_update_t`、FENCE.I pulse，以及提交追踪接口。空 ROB 中断 PC 由内部 `retire_next_pc` 提供。
...
- Core 级 ADDI/ADD/LW/SW 单项测试，以及 Load 请求穿过 Core 外 DMEM 寄存级。
...
6. `core_top.sv` 已连接 IF/ID 与完整 Backend，但尚未加入行为内存、Cache 或 SoC 地址/中断系统。
