...
// Backend integration boundary with INT/Branch/CSR plus the conservative LSU.
// MDU dispatch remains backpressured for the next integration step.
module backend_lsu_cluster #(
...
`ifndef SYNTHESIS
  always_ff @(posedge clk_i) begin
    if (!rst_i) begin
      assert (!int_push_valid_q[0] || int_iq_push_ready)
        else $error("backend_lsu_cluster pushed INT IQ while not ready");
      assert (!mem_push_valid_q[0] || mem_iq_push_ready)
        else $error("backend_lsu_cluster pushed MEM IQ while not ready");
      ...
      if (branch_event_raw.valid)
        assert (!branch_event_pending_q || branch_event_fire)
          else $error("backend_lsu_cluster branch resolve queue overflow");
    end
  end
`endif
