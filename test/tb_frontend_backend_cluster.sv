...
module tb_frontend_backend_cluster;
  timeunit 1ns;
  timeprecision 1ps;
  ...
  task automatic wait_prf_write_data(
    ...
    begin
      ...
      while (!seen) begin
        @(posedge clk_i); #1;
        if (dut.u_backend.wb_valid[0] &&
            dut.u_backend.wb_completion[0].valid &&
            dut.u_backend.wb_completion[0].write_prf &&
            (dut.u_backend.wb_completion[0].data == expected_data) &&
            (dut.u_backend.wb_completion[0].producer == expected_producer)) begin
          observed_prd = dut.u_backend.wb_completion[0].prd;
          seen = 1'b1;
        end
        if (dut.u_backend.wb_valid[1] &&
            dut.u_backend.wb_completion[1].valid &&
            dut.u_backend.wb_completion[1].write_prf &&
            (dut.u_backend.wb_completion[1].data == expected_data) &&
            (dut.u_backend.wb_completion[1].producer == expected_producer)) begin
          observed_prd = dut.u_backend.wb_completion[1].prd;
          seen = 1'b1;
        end
        cycles = cycles + 1;
        if (cycles > 360)
  ...
  initial begin
    ...
    wait_prf_write_data(32'd6, PROD_INT0, prd);
    wait_prf_write_data(32'd7, PROD_INT0, prd);
    wait_prf_write_data(32'd42, PROD_MUL, prd);
    wait_prf_write_data(32'd6, PROD_DIV, prd);
    repeat (8) @(posedge clk_i); #1;

    if (free_lq_count_o != 8 || free_sq_count_o != 8)
      $fatal(1, "frontend/backend memory resources did not drain");

    $display("PASS: frontend_backend_cluster directed tests");
    $finish;
  end
