...
Core 外部现已加入测试专用统一行为内存：IMEM 为一拍同步读，DMEM 请求必须先进入 Core 外的一项寄存器，再访问统一 word array。RV32I、RV32M、CSR/SYSTEM、同步异常和 FENCE/FENCE.I 单项门禁均已通过；当前统一行为内存不建模 Store access fault。组合场景已验证双发射、长 DIV、访存混排、连续分支 recovery，以及连续 CSR、同步异常、机器外部中断和 MRET。组合测试还修复并验证了 Rename FIFO 滞留期间持续接收写回唤醒，避免跨串行边界丢失 CDB。官方 HEX 回归尚未开始。
...
截至 2026-07-01，包含 `core_top.sv` 和统一行为内存的全量编译为 `0 Errors, 0 Warnings`。十四项既有测试、七项 Core 单项测试和五项 Core 组合测试共二十六项全部输出 `PASS`。

## 下一步

自设单项指令门禁和前五项组合测试已经完成。最后一项组合测试为自修改代码与 FENCE.I，重点检查 LSQ 排空、流水清除和新指令重新取回；通过后再进入官方 HEX 回归。
