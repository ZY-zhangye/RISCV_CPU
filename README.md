...
本目录正在实现一个面向 RV32 的乱序、双发射处理器。目前已完成双取指、RV32I/M 双路译码，以及由 `backend_top.sv` 串联的 Rename、ROB/IQ/LSQ、同步 PRF、五类执行单元、WB0/WB1 和最小机器态 CSR/精确提交通路。
...
截至 2026-06-30，上述文件已通过 QuestaSim 编译：`0 Errors, 0 Warnings`。全部十四项定向与集成测试均输出 `PASS`。

## 下一步

建议下一步将 `backend_top.sv` 与 IF/ID、I-cache/D-cache 和 SoC 中断源连接，并增加随机指令流与差分验证。
