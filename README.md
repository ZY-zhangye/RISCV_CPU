...
└─ test/
   ...
   ├─ tb_core_combo_branch_recovery.sv
   ├─ tb_core_combo_dual_issue.sv
   ├─ tb_core_combo_fence_i_smc.sv
   ├─ tb_core_combo_long_div.sv
   ├─ tb_core_combo_memory.sv
   ├─ tb_core_combo_trap_interrupt.sv
   ├─ tb_core_csr_system_instructions.sv
   ├─ tb_core_exception_fence.sv
   ├─ tb_core_rv32m_instructions.sv
...
- AGU 请求被选中时即标记 `address_issued`，避免 issue1/execute ready 反馈进入 LSQ 条目状态写入路径；
- Store-to-Load forwarding 先寄存 load 槽位和最年轻覆盖 store 槽位，下一拍再取数并格式化，切断年龄扫描到 32 位转发数据的组合路径；
...
Core 外部现已加入测试专用统一行为内存：IMEM 为一拍同步读，DMEM 请求必须先进入 Core 外的一项寄存器，再访问统一 word array。RV32I、RV32M、CSR/SYSTEM、同步异常和 FENCE/FENCE.I 单项门禁均已通过；当前统一行为内存不建模 Store access fault。六项组合场景已全部通过，覆盖双发射、长 DIV、访存混排、连续分支 recovery、CSR/异常/中断/MRET，以及自修改代码与 FENCE.I。组合测试修复了 Rename FIFO 跨串行边界丢失 CDB，以及 FENCE.I 早于 Core 外 Store 寄存级可见的问题。官方 HEX 已精简为物理版 RV32I/M/F、Zicsr `csr/mcsr` 和 Zba 三项，其中当前必过范围为 RV32I、RV32M 与 `rv32mi-p-csr`；正式回归尚未开始。
...
RV32M 译码由 `tb_id_decode_m.sv` 定向验证，完整 Core 执行与提交由 `tb_core_rv32m_instructions.sv` 覆盖；物理寄存器堆由 `tb_physical_regfile.sv` 验证。
...
截至 2026-07-02，包含 `core_top.sv`、统一行为内存和官方 HEX testbench 的 RTL 回归通过：`scripts/run-regression.ps1 -Mode unit` 为 27/27 `PASS`，`-Mode all` 为 77 `PASS`、16 `SKIP`。SKIP 项为尚未实现的 RV32F/Zba、`mcsr` 和非对齐数据访问策略项。
...
当前 FPGA 目标为 Kintex-7 `xc7k325tffg900-2`，PLL `clk_out1` 按 200 MHz / 5.000 ns 约束评估。2026-07-02 的非 bitstream 综合、布局和 `phys_opt_design` 后报告为：

- `fpga_impl/output/top_timing_summary_current.rpt`：WNS `-7.680ns`，TNS `-98341.588ns`；
- 最差路径：`rob_head_tag_iq0_reg[0]` 到 LSQ `memory_request_reg[write_data][24]/CE`，data path `12.168ns`，29 logic levels，route 约 85%；
- 本轮前一版 WNS 为 `-8.073ns`，再前一版为 `-8.499ns`。目前主瓶颈已从 `CSR/BRU -> LSQ address_issued` 移到 LSQ memory select / store-drain 全相联年龄扫描。

结论：200 MHz 尚未收敛，后续应继续拆分 LSQ memory select，特别是 `rob_head_tag -> committed store/load select -> memory_request_reg CE` 和 `entries[*].rob_tag -> memory_select_q_store_idx` 两类路径。

## 下一步

既有 RTL 门禁和当前官方允许清单均已完成。`rv32ui-p-ma_data`、`rv32mi-p-mcsr`、RV32F 和 Zba 继续按计划明确标记为 `SKIP`；下一阶段若以 200 MHz 为目标，应优先继续 LSQ 时序流水化，再进入 Cache/SoC 集成或随机差分验证。
