...
本目录正在实现一个面向 RV32 的乱序、双发射处理器。目前已完成双取指、RV32I/M 双路译码、双路 Rename、同步 4R2W 物理寄存器堆、32 项 ROB、组合 Dispatch、双分区 IQ 和独立乱序 LSQ。
...
建议下一步连接 PRF/广播操作数选择级，并实现 LSU 地址生成和两组执行/写回接口。
